...
import static scheme.SchemeVars.build;
...
public interface ModedInputHandler {
    ...
    public default void flushBuildingTools() {
        if (build.mode != Mode.remove) {
            flush(build.plan); 
        }else {
            build.plan.each(player.unit()::addBuild);
        }
        build.plan.clear();
    }
    ...
    public default void drawSize(int x1, int y1, int x2, int y2, int maxLength) {
        ...
        ui.showLabel(x + ", " + y, -1, 0.02f, x2 * tilesize + sizeX, y2 * tilesize + sizeY);
    }
